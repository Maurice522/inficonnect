...
const Discover = ()=>{
    ...
    const deleteComment =(postid,commentid)=>{
      fetch(`/discover/deletecomment/${postid}/${commentid}`,{
        method:"delete",
        headers:{
          Authorization:"Bearer "+localStorage.getItem("jwt")
        }
          ...
          return(
            <div className="card home-card" key={item._id}>
              ...
              {item.likes.includes(state._id)?
                ...
                :
                <span><span className="numbers" style={{position:"relative",bottom:"4px",fontSize:"22px"}}>{item.likes.length} </span><i className="fas fa-handshake" style={{fontSize:"24px"}} onClick={()=>{likePost(item._id)}}></i></span>}
